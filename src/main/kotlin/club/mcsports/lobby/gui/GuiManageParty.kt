...
class GuiManageParty(party: Party, partyApi: PartyApi.Coroutine) {
    ...
    val gui = buildCombinedInterface {
        ...
        withTransform { pane, view ->
            ...
            pane[2, 5] = StaticElement(drawable(ItemComponents.TRASH_BIN.build().also { item ->
                ...
                InterfacesConstants.SCOPE.launch {
                    partyApi.getInteraction().deleteParty(view.player.uniqueId)
                }
                view.player.sendMessage(miniMessage("䶵 <white>You've <color:#dc2626>deleted</color> your party."))
            }
        }

        // TODO: Implement party invite toggle
        withTransform(interfaceProperty(party.settings.allowInvites)) { pane, view ->
            pane[2, 3] = StaticElement(drawable(PartyItemComponents.PARTY_INVITES.build().also { item ->
                ...
                // TODO
                view.player.sendMessage(miniMessage("<red>Not implemented yet"))
                // party.canInvite = !party.canInvite
                view.redrawComplete()
            }
        }

        withTransform(interfaceProperty(party.settings.isPrivate)) { pane, view ->
            pane[2, 4] =
                StaticElement(drawable(((if (party.settings.isPrivate) ItemComponents.PADLOCK_LOCKED else ItemComponents.PADLOCK_UNLOCKED).build()).also { item ->
                    item.editMeta {
                    ...
                    // TODO
                    view.player.sendMessage(miniMessage("<red>Not implemented yet"))
                    // party.isPrivate = !party.settings.isPrivate
                    view.redrawComplete()
                }
        }
